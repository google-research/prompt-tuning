...
class MultiTaskFactorizedPrompt(nn.Module):
  """Geneate a MultiTaskFactorizedPrompt and concatenate it with the input.
  ...
  module then slices off the language and task indices from the input and
  concatenates the prompt. This can be used in conjunction with the
  `multitask=True` arguments for attention mask creation to do multi-task
  prompting without need multitask subclasses of various flaxformer modules.
  ...
  def __call__(self, x, x_embed):
    ...
    # Remove the language and task index tokens
    x_embed = x_embed[:, 2:]
